...
=================================================================================
Anchor = FROZEN Stage-1 prediction (never updated). It pins down the calcium
the Stage-1 model is CONFIDENT about, while leaving room for the score loss to
ADD missed calcium (e.g. under-segmented large lesions) and REMOVE false
positives (stents, etc.).

Key design choices that resolve the score-vs-anchor tug of war:
  * frozen anchor (not EMA) -> stable, high-quality reference (Dice 0.88)
  * confidence gate -> only anchor where Stage-1 prob is high (sure calcium)
  * ASYMMETRIC -> penalize only when student ERASES sure calcium
    (student < anchor); do NOT penalize student for ADDING calcium.
    => score is free to grow the mask on big lesions; anchor only stops it
       from deleting calcium it already found.

Forces on the student:
  L_score  : DiffAgatston(M) vs GT score, log space      (total)
  L_anchor : asymmetric "don't erase sure calcium" vs frozen Stage-1   (NEW)
  L_HU     : penalize prob where HU < 130
  L_heart  : penalize prob outside heart mask
  L_TV     : smoothness
"""
...
def asymmetric_anchor(prob_student, prob_anchor, conf=0.5):
    """penalize ONLY where the frozen anchor is confident calcium
    (prob_anchor > conf) AND the student is trying to erase it
    (student < anchor). Adding calcium is NOT penalized."""
    sure = (prob_anchor > conf).float()                 # sure-calcium mask
    deficit = F.relu(prob_anchor - prob_student)        # >0 only if erasing
    num = (deficit ** 2 * sure).sum()
    den = sure.sum() + 1e-6
    return num / den
...
def train_stage2a(cfg):
    ...
    # FROZEN anchor = a separate copy of Stage-1, never updated
